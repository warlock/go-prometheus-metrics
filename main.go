...
var (
	c = promauto.NewCounter(prometheus.CounterOpts{
		Name: "josep_app_sample_metric",
		Help: "Sample metric for Josep",
	})

	h = promauto.NewHistogram(prometheus.HistogramOpts{
		Name: "josep_app_sample_histogram",
		Help: "Sample histogram for Josep",
	})

	d = promauto.NewCounterVec(prometheus.CounterOpts{
		Name: "josep_app_sample_devices",
		Help: "Sample counter opts devices for Josep"}, []string{"device"})
)
